...
%convert the code above to comment
[m,n]=size(data);
...
data.FlightStatus(t)='ontime'; %convert to comment if changing in the data is not allowed
...
for i=0:x
    ...
    error(i+1,1)=i;error(i+1,2)=e;
end
plot(error(:,1),error(:,2));
title('Prune Level vs Error Rate');
[Y,I]=min(error(:,2));
prunedTree = prune(model, 'Level', I-1);
cmvalid = confusionmat(valid.y56,predict(prunedTree,valid));
e=(cmvalid(1,2)+cmvalid(2,1))/size(valid,1);
view(prunedTree,'Mode','Graph');
totalcost=cmvalid(1,2)*50+cmvalid(2,1)*5;